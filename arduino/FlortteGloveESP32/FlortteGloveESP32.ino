...
void captureBentPose() {
  if (!isCalibrating) beginWebCalibration();
...
void captureStraightPose() {
  ...
  for (int i = 0; i < FINGER_COUNT; i++) {
    ...
    if (abs(straightValues[i] - bentValues[i]) >= MIN_CALIBRATION_RANGE) {
...
void handleCalibrate() {
  ...
  if (action == "start") beginWebCalibration();
  else if (action == "bent") captureBentPose();
  else if (action == "open") captureStraightPose();
  else if (action == "cancel") isCalibrating = false;
  else {
...
void setup() {
  ...
  Serial.println("Open Diagnostics and complete calibration on the website.");
